...
def resample(spk, origin_wavpath, target_wavpath):
    ...
    #     print(wavfiles)
    for wav in wavfiles:
        # folder_to = join(target_wavpath, spk).replace("/","\\")
        # print(folder_to)
        # os.makedirs(folder_to, exist_ok=True)

        wav_to = join(target_wavpath, spk, wav).replace("/","\\")
        wav_from = join(origin_wavpath, spk, wav).replace("/","\\")

        # print(wav_to , wav_from,"\n\n\n")
...
def resample_to_16k(origin_wavpath, target_wavpath, num_workers=1):
    ...
    for spk in spk_folders:
        # print(spk, origin_wavpath, target_wavpath)
...
if __name__ == '__main__':
    ...
    origin_wavpath_default = "./src_audio"
    target_wavpath_default = "./src_audio"
    mc_dir_train_default = './src_feature'
    mc_dir_test_default = './src_feature'
    ...
    speaker_used = ['225','262', '272', '229', '232', '292', '293', '360', '361', '248', '251']
